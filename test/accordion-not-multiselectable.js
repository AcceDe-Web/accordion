...
// test 29
test( '29| Un « Click » sur une seconde entête de panneau ferme le précédent panneau ouvert', async t => {
  await page.reload();
  await page.click( '#tab1' );
  await page.click( '#tab2' );
  const result = await page.evaluate(() => {
  ...
  t.equal( result.aria1Expanded, 'false', 'L’élément précédent doit être actif.' );
  t.equal( result.aria1Hidden, 'true', 'L’élément précédent doit être affiché.' );
  t.equal( result.aria1CB, 'true', 'La fonction de calback de fermeture ne doit pas être exécuté sur l’élément précédent.' );
